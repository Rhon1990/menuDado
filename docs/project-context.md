...
1. Crear menús y decidir qué comer hoy.
   ...
   - Las nuevas contribuciones de `sharedAiMenus` incluyen `identityVersion: 2`; las reglas rechazan nuevas identidades antiguas sin bloquear el guardado privado. Al leer, la app vuelve a canonicalizar los hasta 12 candidatos y agrupa tanto identidades exactas como recetas de la misma familia con un solapamiento conceptual mínimo de `0.80` antes de aplicar la rotación. `qa/migrate-ai-menu-hive-v2.mjs` consolida automáticamente solo identidades exactas con `dry-run` por defecto, backup obligatorio y confirmación exacta del proyecto; al aplicar relee y bloquea transaccionalmente cada grupo para conservar `eligibilityKeys` concurrentes antes de escribir el canónico y eliminar copias. Las coincidencias basadas únicamente en el umbral se informan para revisión y nunca se eliminan automáticamente.
