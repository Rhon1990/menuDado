...
1. Crear menús y decidir qué comer hoy.
   ...
   - Si la generación en vivo supera 12 segundos, el dado continúa girando y el texto indica que la IA está tardando más de lo habitual. Solo cuando la petición real termina con error, MenuDado consulta hasta 12 propuestas de `sharedAiMenus` con la misma clave exacta de idioma, tipo, público y perfil; vuelve a validar localmente restricciones, alergias y seguridad antes de mostrar una alternativa. Nunca relaja el perfil para conseguir un resultado.
   - La colmena de respaldo es anónima y separada de `users/{uid}`. Solo recibe una propuesta creada por IA cuando el usuario la guarda sin modificar; no guarda UID, correo, foto, ingredientes base escritos ni el contenido legible del perfil. Las recetas recuperadas no vuelven a contribuir.
   - Los equivalentes culinarios se deduplican mediante una clave semántica canónica incluida en la misma respuesta de Gemini y un hash SHA-256, por ejemplo pasta con tomate y espaguetis en salsa de tomate. Esto no añade una segunda llamada de IA.
   - El fallback se controla con Remote Config `ai_menu_hive_enabled`. Si está desactivado no hace lecturas ni escrituras. Una colmena vacía, sin conexión o sin coincidencia segura conserva el error original: no existe garantía absoluta en una primera instalación sin datos compatibles.
...
4. Análisis saludable con IA.
   ...
   - Generación y análisis comparten un único contador local de llamadas reales al proveedor. El tramo gratuito diario es de 10 llamadas para una cuenta registrada y 5 para un invitado cuando `guest_ai_limits_enabled=true`; el contador solo decrece al iniciar una llamada real a Gemini y no al validar, abrir el anuncio, cerrar el anuncio o recuperar una receta de la colmena.
   - Al agotar el tramo gratuito, solo la generación de ideas puede obtener hasta 10 intentos adicionales diarios mediante anuncios bonificados opcionales. Cada recompensa completada concede exactamente un intento y reanuda el flujo normal: primero Gemini y, si falla, la colmena compatible. El análisis individual o por lote nunca consume recompensas ni ofrece anuncios.
   - Existe un límite local absoluto de 20 llamadas reales al proveedor por día. Por ello una cuenta puede alcanzar 10 gratuitas + 10 bonificadas y un invitado 5 gratuitas + 10 bonificadas. Alcanzado el máximo aplicable, el dado muestra el límite diario y no ofrece otro anuncio.
...
6. Comportamiento local primero con backend.
   ...
   - En modo invitado, MenuDado permite usar el dado local sin límite, editar y borrar menús, y marcar favoritos. `guest_limits_enabled` controla únicamente el límite diario local de 5 menús guardados. El uso de Gemini se rige por el contador compartido de IA: 5 llamadas gratuitas para invitado y 10 para cuenta registrada cuando `guest_ai_limits_enabled=true`, más las recompensas opcionales permitidas solo para generar ideas.
...
- Reglas Firestore: `sharedAiMenus/{semanticHash}` permite lectura autenticada y creación con esquema saneado; impide sobrescribir o borrar recetas y solo permite añadir claves de compatibilidad. Las reglas se validan con el emulador local.
...
- Remote Config:
  ...
  - `guest_limits_enabled` controla únicamente el límite diario de menús guardados por el invitado; por defecto local vale `true`.
  - `guest_ai_limits_enabled` controla el tramo gratuito del contador compartido de llamadas IA (5 invitado, 10 cuenta registrada); por defecto local vale `true`.
...
- Publicidad:
  ...
  - Al agotar el tramo gratuito de IA, el dado ofrece voluntariamente un anuncio bonificado. La recompensa es un único intento adicional de generación, con máximo local de 10 recompensas diarias y límite de frecuencia de AdMob de 10 impresiones por usuario y día. Cerrar o no poder cargar el anuncio no concede crédito ni consume una llamada IA.
  - Durante desarrollo y en `releaseDebuggable` se usan App ID y bloques demo de Google para evitar tráfico inválido en AdMob. El bloque bonificado de producción se inyecta con `menudadoRewardedAiAdUnitId` o `MENUDADO_REWARDED_AI_AD_UNIT_ID`; si falta, la función queda cerrada de forma segura.
