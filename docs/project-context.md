...
1. Crear menús.
   ...
   - La pausa local de IA debe sentirse corta y solo proteger contra doble tap o llamadas casi simultáneas; no debe bloquear varias ideas seguidas durante decenas de segundos. Si vuelve a aparecer `IA descansando` tras solo 1 o 2 ideas, revisar primero `AI_REQUEST_THROTTLE_MILLIS`, el estado `isAiRequestThrottlePause` y que no se esté confundiendo una pausa local con una cuota real de Firebase/Gemini.
