...
1. Crear menús y decidir qué comer hoy.
   ...
   - Los equivalentes culinarios se deduplican mediante una clave semántica incluida en la misma respuesta de Gemini y una identidad local v2 antes del hash SHA-256. La canonicalización es específica por familia, ingredientes y preparación: ordena ingredientes y unifica alias conservadores como `vegetable`/`vegetables` y `mix`/`mixed` sin aplicar stemming genérico que pueda juntar recetas distintas. Las instrucciones técnicas del prompt sustituyen cuatro líneas anteriores por una más corta; no añaden llamadas, campos JSON ni longitud al prompt.
   - Las nuevas contribuciones de `sharedAiMenus` incluyen `identityVersion: 2`; las reglas rechazan nuevas identidades antiguas sin bloquear el guardado privado. Al leer, la app vuelve a canonicalizar y agrupa documentos heredados equivalentes antes de aplicar la rotación, por lo que no los muestra como recetas diferentes. `qa/migrate-ai-menu-hive-v2.mjs` permite consolidar los documentos existentes con `dry-run` por defecto, backup obligatorio antes de aplicar, confirmación exacta del proyecto, relectura del documento canónico y eliminación posterior de las copias.
   - Cada invitado y cada cuenta mantienen en el teléfono una rotación independiente de colmena con un máximo de 24 `semanticHash`, sin guardar recetas ni datos de perfil. Dentro de los hasta 12 candidatos compatibles consultados se muestran primero los no vistos; al completar el ciclo se evita repetir inmediatamente si existe otra alternativa. Este historial no se sincroniza, no se incluye en copias de seguridad ni aumenta las lecturas de Firestore.
...
4. Análisis saludable con IA.
   ...
   - Existe un límite local absoluto de 20 llamadas reales al proveedor por día, independiente de los saldos de producto. Al agotar las 10 recompensas del ámbito activo, el dado deja de ofrecer anuncios en ese ámbito; haber alcanzado antes las 20 llamadas técnicas no elimina saldos de otro ámbito y las generaciones todavía autorizadas intentan resolverse con la colmena.
