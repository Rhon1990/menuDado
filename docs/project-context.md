...
1. Crear menús y decidir qué comer hoy.
   ...
   - Si el dado de IA está bloqueado por falta de tipo de comida, público objetivo o pausa de IA, el propio bloque del dado debe explicar el motivo fuera del botón del dado, usar un aviso cálido alineado con la paleta de MenuDado dentro del bloque amarillo, mantener el botón en un color neutral de marca claramente distinto del naranja activo y recordar que el usuario puede cambiar a `Escribir menú`. El aviso usa el naranja del dado solo como acento/título y deja el motivo en texto oscuro para mejorar legibilidad. Cuando `aiRetryAtMillis` representa throttle o backoff, el mensaje debe ser genérico: `La IA necesita un momento. Espera un poco o escribe tu menú.`; no debe afirmar que se agotó la cuota.
   - Cuando la IA completa la idea, se abre el modal `Una idea pensada para ti` con nombre, descripción, notas, calorías, análisis saludable y sugerencia. El primer metadato identifica la inspiración culinaria seleccionada localmente (`Cocina mexicana`, `Cocina mediterránea`, etc.), seguido del estado saludable y las calorías en una fila responsive que puede saltar de línea. Al guardar la idea, esta inspiración se conserva como metadato opcional del menú en Room y Firebase para mostrarla después; no añade llamadas, tokens ni campos al JSON de Gemini. `Guardar en mis menús` es la acción principal; `Probar otra idea` reutiliza tipo, público e ingredientes base y respeta cuota, throttle y timeout; `No guardarla` sigue disponible como acción terciaria.
   ...
   - Tras persistir correctamente un menú nuevo en Room, la app muestra de inmediato un Snackbar breve y localizado confirmando `Menú guardado`; las validaciones bloqueadas o los límites de invitado no deben mostrar esa confirmación. La subida pendiente a Firestore continúa como tarea cancelable y no retrasa el feedback local-first cuando la red está lenta. Las mutaciones remotas del mismo menú se serializan por ID para que un guardado pendiente nunca sobrescriba una edición o eliminación posterior.
   - Desde la acción `Crear una idea con IA` se puede volver al modo de generación según el tipo seleccionado.
   ...
   - Si al editar un menú se cambia nombre, tipo, público objetivo, descripción o notas, el análisis IA y las calorías asociadas se descartan para evitar mostrar una evaluación desactualizada; el usuario puede volver a tocar `Revisar con IA`.
...
3. Dado contextual.
   - El dado aparece dentro del bloque `¿No sabes qué cocinar?` y pertenece exclusivamente a la generación de una idea saludable nueva con IA. `Elegir un menú al azar` ejecuta la selección local sobre los guardados con un indicador de progreso propio y no anima el dado IA ni consume cuota de IA.
   ...
   - Los selectores de tipo y público del bloque `¿No sabes qué cocinar?` se usan tanto para generar con IA como para elegir menús guardados.
   ...
   - El resultado se presenta como `Hoy toca`.
...
4. Análisis saludable con IA.
   ...
   - En tarjetas sin análisis, la fila de acciones debe mostrar `Revisar con IA` ancho y una papelera compacta solo con icono rojo. En tarjetas ya analizadas, debe mostrarse un botón `Eliminar` de ancho completo con icono de papelera rojo y texto.
   - Cuando existan menús pendientes de análisis pertenecientes a públicos activos, la lista debe ofrecer una acción general `Revisar menús pendientes` para evaluar un lote pequeño de menús visibles en una sola llamada a Gemini y reducir consumo de requests. El análisis individual por tarjeta debe mantenerse para cuando el usuario quiera evaluar un único menú.
   ...
   - Los botones visibles de IA deben mostrar entre paréntesis los usos locales restantes del día solo cuando la IA está disponible, por ejemplo `Lanza el dado (19)` y `Revisar con IA (19)`. En modo invitado con `guest_limits_enabled=true`, el contador visible debe corresponder al límite de invitado de esa acción: generación de ideas y análisis muestran sus propios restantes sobre 5, aunque la cuota técnica del proyecto siga siendo 20. Si hay una pausa activa, no deben mostrar el contador diario; deben mostrar `La IA necesita un momento` en una sola línea, recortado con puntos suspensivos si el ancho no alcanza, sin afirmar que se agotó la cuota.
   ...
   - `aiRetryAtMillis` se usa tanto para throttle local como para backoff del proveedor, por lo que el estado o contador visible de espera es genérico y no promete causa ni reactivación exacta. El mensaje de fallo sí puede indicar una causa cuando el proveedor permite clasificarla con certeza. Si devuelve `retry in`, usar ese valor solo internamente; si no, usar el siguiente reset diario de Gemini API a medianoche Pacific Time cuando corresponda.
   - `aiLocalDailyLimitMessage` sí representa la cuota diaria gratuita real y debe informarla con claridad, sin prometer «mañana» porque el reinicio ocurre a medianoche Pacific Time: `Ya agotaste los usos gratuitos de IA de hoy. Tus menús siguen disponibles; vuelve a intentarlo más tarde.`
   ...
   - Timeout y error genérico no deben recomendar revisar la conexión ni atribuir una causa que el clasificador no haya confirmado. Sus recuperaciones neutrales son `La IA tardó más de lo esperado. Inténtalo de nuevo.` y `La IA no pudo completar la solicitud. Inténtalo de nuevo más tarde.`, con equivalentes naturales en inglés y francés.
...
5. Lista de mercado.
   ...
   - Solo la IA crea productos: una idea generada los recibe en la misma llamada; un menú escrito manualmente los recibe al tocar `Revisar con IA`, junto con el análisis saludable. No hay entrada manual de productos.
...
- Una acción principal clara: `¿No sabes qué cocinar?`, con `Lanza el dado` visible por defecto, `Elegir un menú al azar` como secundaria y escritura manual como terciaria.
...
- Validar que `Lanza el dado` es la acción inicial, que abre `Una idea pensada para ti` sin guardar automáticamente, que `Guardar en mis menús` persiste análisis y calorías y que `Probar otra idea` conserva la intención sin saltarse protecciones.
