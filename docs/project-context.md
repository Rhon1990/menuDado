...
- Fuente de verdad: para la configuración Android prevalecen `app/build.gradle.kts` y el manifiesto final del artefacto sobre los datos históricos de este documento.
...
- Publicación Play Store:
  ...
  - Política de nivel de API de destino: la rama actual ya declara Android 16/API 36. Esto confirma la configuración local, pero Google Play solo retirará el aviso después de publicar y procesar en producción un AAB que conserve `targetSdk=36` o superior. La captura de Play Console puede corresponder al último artefacto productivo procesado y no demuestra que el código actual siga orientado a API 35.
  ...
  - La compatibilidad edge-to-edge se introdujo para Android 15/API 35: `MainActivity` habilita edge-to-edge con `androidx.activity` actualizado y fuerza `LAYOUT_IN_DISPLAY_CUTOUT_MODE_ALWAYS`; debe seguir validándose al orientar la aplicación a Android 16/API 36. Las fotos de menús se decodifican con `BitmapFactory.Options.inSampleSize` para evitar cargar bitmaps completos en portadas.
